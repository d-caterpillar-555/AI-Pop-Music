...
Avo.configure do |config|
  ...
  ## == Authentication ==
  # The admin panel is staff-only. Without this block Avo mounts with no
  # authentication at all, which would publish the catalogue, every customer
  # record and every licence to anyone who guessed the path.
  config.current_user_method = :current_user
  config.authenticate_with do
    authenticate_user!

    unless current_user&.admin? || current_user&.editor?
      redirect_to main_app.root_path, alert: "You are not allowed to do that."
    end
  end
  ...
  #   sound_button: false, # show the mute toggle in the switcher
